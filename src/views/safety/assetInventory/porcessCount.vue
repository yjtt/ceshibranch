    ...
    <div class="content">
        ...
        <el-table
          ...
          <!-- <el-table-column
            ...
            <el-table-column
              ...
              >
              <template slot-scope="scope">
                {{scope.row.process_path==""?"--":scope.row.process_path}}
              </template>
            </el-table-column>
            <el-table-column
              :show-overflow-tooltip = 'true'
              label="启动参数" >
              <template slot-scope="scope" >
                {{scope.row.init_parameters==""?"--":scope.row.init_parameters}}
              </template>
            </el-table-column>
            <el-table-column
              label="启动时间"
               width="170">
              <template slot-scope="scope">
                {{scope.row.init_time==""?"--":scope.row.init_time}}
              </template>
            </el-table-column>
            <el-table-column
              label="运行用户"
            ...
            <el-table-column
              ...
              >
              <template slot-scope="scope">
                {{scope.row.ppname==""?"--":scope.row.ppname}}
              </template>
            </el-table-column>
               <el-table-column
              label="上报时间"
      ...
      <div class="template_custom" v-show="!isDefault" v-loading='loading_H'>
        ...
        <el-table
            ...
            <el-table-column
              label="变动状态" :show-overflow-tooltip = 'true'>
              <template slot-scope="scope">
                <!-- {{scope.row.status}} -->
            ...
            <el-table-column
              label="进程路径" :show-overflow-tooltip = 'true'>
              <template slot-scope="scope">
                {{scope.row.process_path==""?"--":scope.row.process_path}}
              </template>
            </el-table-column>
            <el-table-column
              label="启动参数" :show-overflow-tooltip = 'true'>
              <template slot-scope="scope" >
                {{scope.row.init_parameters==""?"--":scope.row.init_parameters}}
              </template>
            </el-table-column>
            ...
            <el-table-column
              label="运行用户" width="80" :show-overflow-tooltip = 'true'>
              <template slot-scope="scope">
                {{scope.row.user==""?"--":scope.row.user}}
              </template>
            </el-table-column>
            ...
            <el-table-column
              label="父进程" :show-overflow-tooltip = 'true'>
              <template slot-scope="scope">
                {{scope.row.parent_process_name==""?"--":scope.row.parent_process_name}}
              </template>
            </el-table-column>
               <el-table-column
              label="上报时间"
...
import { processEventService, serverRegion, portEventService  } from "@/api/safety";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    getexceptionRegionNum(zoneAll) {
      let ser1 = "remoteGetProcessZoneList";
      let paramData = "<type>" + this.zoneType + "</type>";
      processEventService(ser1, paramData)
        .then(res => {
    ...
    getNewestProList() { 
      console.log(1);
      this.loading_N = true;
      let ser = "getProcessNewEventByAdminId",
        paramData =
          ...
          "</onePageSize>";
      processEventService(ser, paramData)
        .then(res => {
    ...
    getHistorytProList() {
      ...
      let ser = "getProcessHistoryEventByAdminId",
        paramData =
          "<areaZoneId>"+this.currentId+"</areaZoneId><condition>" +
          this.search_his_pro +
          "</condition><page>" +
