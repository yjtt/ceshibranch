...
import { processEventService, serverRegion, portEventService,softManage  } from "@/api/safety";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    getexceptionRegionNum(zoneAll) {
      let ser1 = "remoteGetSoftWareZoneList";
      let paramData = "<type>" + this.zoneType + "</type>";
      softManage(ser1, paramData)
        .then(res => {
    ...
    getNewestProList() { 
      console.log(1);
      this.loading_N = true;
      let ser = "getSoftWareNewEventByAdminId",
        paramData =
    ...
    getHistorytProList() {
      ...
      let ser = "getSoftWareHistoryEventByAdminId",
        paramData =
          "<areaZoneId>"+this.currentId+"</areaZoneId><condition>" +
          this.search_his_pro +
          "</condition><page>" +
