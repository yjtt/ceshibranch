                  ...
                  <el-menu :collapse='isOpen' class='aside-width' :class="{'el-menu-collapse' : isOpen}" :default-active="''+5">
                    ...
                    <el-tooltip v-for='(items ,index) in navListTwo' :key='index' :disabled="!isOpen" :enterable='false' :content="items.name" placement="right">
                        <el-menu-item @click="showListDetail(index)" :index='""+index'>
                            <i class="aside-icon"><svg-icon :icon-class='items.id'></svg-icon></i>
                    ...
                    <!-- {{navList}} -->
                    <!-- <el-tooltip :disabled="!isOpen" :enterable='true'  v-for = '(item,index) in navList' :key='index+1'  :content="item.name" placement="right">
                        <el-menu-item  @click='showListDetail(index)' v-if="!item.hide" :index="item.id">
        ...
        methods:{
            ...
            isTab(){
                ...
                if(this.$route.name === 'safety'){
                    ...
                    //sll添加
                    // alert(window.location.hash);//获取哈希值
                    // window.location.hash = '#'+this.subList[0].path;
                    // console.log(this.subList,3333333333333333333333);
                    // for(var i=0;i<this.subList.length;i++){
                    //     console.log(i,686868686868686868)
                    //     console.log(this.subList[i].path);
