...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            jumpPush(index){
                ...
                for(var i=0;i<this.navListTwo.length;i++){
                    if(this.navListTwo[i].type==0 && index == i){
                        window.location.href= this.navListTwo[index].url
                    }else if(this.navListTwo[i].type==1 && index == i){
                        window.open(this.navListTwo[index].url)
                    }
                }
            ...
            showListDetail(index){
                ...
                console.log(21333231,this.navList, this.subList)
            },
            isTab(){
                this.defaultIndex = sessionStorage.getItem('Index');
                // this.showListDetail();  //让下标为5的有一个默认样式
        ...
        computed: {
            ...
            navList(){
                console.log(564321,this.$store.getters.regionType);
                let regionType = this.$store.getters.regionType;
                //console.log(list)
                let _ = this._;
